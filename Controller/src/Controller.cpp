...
bool	Controller::readEvent()
{
	...
	if (ret > 0 && FD_ISSET(_joyFd, &fds)){
		...
		std::cout << "Controller event received, bytesRead: " << bytesRead << std::endl;

		if (bytesRead == -1){
			std::cerr << "Error reading joystick event: " << strerror(errno) << std::endl;
			return false;
		}
			...
			switch (event.type){
				case JS_EVENT_AXIS:{
					if (event.number < MAX_AXES){
						...
						std::cout << "Axis " << (int)event.number << " value: " <<
							_normalizedAxes[event.number] << std::endl;
					}
					_doEvent = true;
					break;
				}
