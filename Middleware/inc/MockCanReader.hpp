...
public:
  MockCanReader()
      : _receiveData{0}, _receiveLength(0), _canId(0x100),
  ...
  void setCanId(uint16_t id) { _canId = id; }

  void setShouldReceive(bool shouldReceive) { _shouldReceive = shouldReceive; }
  ...
  uint16_t getId() override { return _canId; }
