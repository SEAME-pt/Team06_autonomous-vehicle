...
private:
    std::map<int, std::pair<int, int>> servo_pwm_values; // channel -> (on_value, off_value)
...
public:
    MockFServo()
        : angle(0), initialized(false), i2c_opened(false), simulate_i2c_failure(false) {}
    ...
    void set_steering(int newAngle) override {
        ...
        if (clamped_angle < 0) {
            // Negative angle (left)
            pwm_value = static_cast<int>(servo_center_pwm + (static_cast<float>(clamped_angle) / max_angle) * (servo_center_pwm - servo_left_pwm));
        }
        else if (clamped_angle > 0) {
            // Positive angle (right)
            pwm_value = static_cast<int>(servo_center_pwm + (static_cast<float>(clamped_angle) / max_angle) * (servo_right_pwm - servo_center_pwm));
        }
        else {
            // Center position
    ...
    void setSimulateI2cFailure(bool simulate) {
        simulate_i2c_failure = simulate;
    }
