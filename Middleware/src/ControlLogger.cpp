...
ControlLogger::ControlLogger(const std::string& log_file_path) {
    std::cout << "Initializing control logger with file: " << log_file_path << std::endl;
    ...
    if (!log_file.is_open()) {
        std::cerr << "Failed to open control log file: " << log_file_path << std::endl;
        throw std::runtime_error("Failed to open control log file: " + log_file_path);
    }
    ...
    if (!log_file.good()) {
        std::cerr << "Failed to write to control log file after opening" << std::endl;
        throw std::runtime_error("Failed to write to control log file after opening");
    }
...
ControlLogger::~ControlLogger() {
    if (log_file.is_open()) {
        try {
            ...
            std::cerr << "Error during control logger shutdown: " << e.what() << std::endl;
        }
    }
}

void ControlLogger::logControlUpdate(const std::string& command, double steering, double throttle) {
    try {
        std::lock_guard<std::mutex> lock(log_mutex);
        if (!log_file.is_open() || !log_file.good()) {
            std::cerr << "Control log file is not open or not in good state" << std::endl;
            return;
        }
