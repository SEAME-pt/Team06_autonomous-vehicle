    ...
    SensorHandler::SensorHandler(
        ...
        : stop_flag(false),
        zmq_c_publisher(c_publisher ? c_publisher : std::make_shared<ZmqPublisher>(zmq_c_address, zmq_context)),
        zmq_nc_publisher(nc_publisher ? nc_publisher : std::make_shared<ZmqPublisher>(zmq_nc_address, zmq_context)),
    ...
    void SensorHandler::addSensor(const std::string& name, std::shared_ptr<ISensor> sensor) {
        std::lock_guard<std::mutex> lock(sensors_mutex);
        if (sensor == nullptr) {
    ...
    std::unordered_map<std::string, std::shared_ptr<ISensor>> SensorHandler::getSensors() const {
        std::lock_guard<std::mutex> lock(sensors_mutex);
        return _sensors;
    }
    ...
    void SensorHandler::sortSensorData() {
        ...
        for (const auto& [name, sensor] : _sensors) {
            ...
            for (const auto& [data_name, data] : sensorDataMap) {
                if (!data) {
                    std::cerr << "Warning: Null SensorData in sensor: " << name << ", data: " << data_name << std::endl;
                    continue;
                }
    ...
    void SensorHandler::start() {
        // Set stop_flag to false regardless of previous value and check if we need to start threads
        bool was_stopped = stop_flag.exchange(false);
        std::cout << "SensorHandler::start() called, was_stopped=" << (was_stopped ? "true" : "false") << std::endl;
        ...
        if (was_stopped ||
            !sensor_read_thread.joinable() ||
            !non_critical_thread.joinable() ||
            !critical_thread.joinable()) {

            std::cout << "SensorHandler::start() - Creating threads" << std::endl;

            // Join any existing threads first (just to be safe)
            if (sensor_read_thread.joinable()) sensor_read_thread.join();
            if (non_critical_thread.joinable()) non_critical_thread.join();
            if (critical_thread.joinable()) critical_thread.join();
    ...
    void SensorHandler::stop() {
        std::cout << "SensorHandler::stop() called, current stop_flag=" << (stop_flag ? "true" : "false") << std::endl;
