...
# Procura pelo endereço IP da interface que está conectada à rede local (geralmente 'eth0', 'wlan0', etc.)
for interface, address in addrs.items():
    for addr in address:
           if addr.address.startswith('10.19'):
                print(f"IP da rede local ({interface}): {addr.address}")
