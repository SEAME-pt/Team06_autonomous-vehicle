...
ZmqPublisher::ZmqPublisher(const std::string& address, zmq::context_t& context, bool test_mode)
    : _context(context),
    ...
    if (!test_mode) {
        try {
            ...
            std::cerr << "ZMQ Error initializing publisher: " << e.what() << std::endl;
            _is_connected = false;
        }
    }
}
...
ZmqPublisher::~ZmqPublisher() {
    if (_is_connected && !_test_mode) {
        try {
            ...
            std::cerr << "Error during ZmqPublisher shutdown: " << e.what() << std::endl;
        }
    }
}
...
void ZmqPublisher::send(const std::string& message) {
    ...
    if (_test_mode) {
        std::cerr << "TEST MODE - PUBLISHING to " << _address << ": " << message << std::endl;
        return;
    }

    // If not connected, try to log the issue but don't crash
    if (!_is_connected) {
        std::cerr << "Error: Cannot send message - publisher not connected" << std::endl;
        return;
    }
    ...
    try {
        ...
        const std::string& msgToSend = message.empty() ? "<EMPTY_MESSAGE>" : message;
